...
description: Implements a plan slice as a running product — stub-through-real plumbing first, tests at database/API/UI, screenshots, a critic pass, CI green, commit, push, tag — without asking the AI Builder for approval between slices. Use when starting Slice A, B or C from lab/plan.md, when the AI Builder says "approved, continue", or when a slice needs its proof and tag.
---

# Delivering slices

A slice is done when the product runs, the proof is green, the story is written, and the remote has it.

## Order inside a slice

1. Re-read the slice in `lab/plan.md` and only that. Change what it names (rule 6).
...
6. Critic pass before the taste review (see `design-loop` for the full loop): taste, density, accessibility — three written critiques in the chapter.
...
- Do not ask "shall I continue to Slice B?" (D-022).
