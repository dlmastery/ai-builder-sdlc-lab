...
const TOUR = [
  ...
  { n: "05", title: "The number you buy: approved without a person, at 1 % error", plate: <PlateGuarantee />, does: "You set an error budget — say, at most 1 field in 100 wrong. It approves on its own only the invoices where every required field clears a bar set to keep that promise, on invoices like yours. Everything else goes to a person, with the reason.", need: "A hundred or so of your invoices, reviewed once, to set the bar for your vendors.", proof: `0 of ${NUMBERS.docs ?? "—"} today — it would not guess a vendor it had never seen. The guarantee is real; the number is honest; the next version trains on exactly that gap.` },
];
      ...
      <section aria-label="Measured" className="border-t border-rule">
        <div className="mx-auto grid w-full max-w-[1200px] gap-8 px-6 py-12 md:grid-cols-4">
          {[
            ...
            [`0/${NUMBERS.docs ?? "—"}`, "approved without a person today, at 1 % error", true],
          ].map(([v, l, red]) => (
