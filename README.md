...
**Want to run this lab with your own product?** Read [PLAYBOOK.md](PLAYBOOK.md) — the meta-script: the script gate by gate (with what this run's AI Builder actually said), what the agent owes you every turn, how to recognise elite artifacts, and what to read before the first training job on a laptop. **Read its §9 first.** This run built and measured the product and then missed the lab's central scene: the AI Builder was grilled once, on day one, and never shown a choice — the plan gate offered three design directions as paragraphs, the agent made every taste call alone for a day, and the AI Builder's verdict on the result was *"you totally spoiled the experience"* ([chapter 12](story/12-the-verdict.md), D-048). The corrected flow — a grilling one question at a time at every gate, references from galleries shown, a fifth gate of at least three **rendered** prototypes for the AI Builder to pick, a second model debating the plan, a taste review on the running screen after every slice, a Series-C bar for the home page — is in the policy file, the skills, and the ▶ rows of the playbook's script table.
...
| [16 — Gate 5, the review screen](story/16-gate-5-review-screen.md) | Three directions for the hero view in one component — the evidence sidebar, the calm document, the inspect canvas — rendered, debated by a second model, picked: B, with a "needs you" strip | `gate-5-review` |
| [17 — The second customer](story/17-customer-test-2.md) | A fresh customer on the rebuilt site, desktop and phone: six broken fixed and re-run (the worst was a silent idempotent upload), eighteen confusing judged one by one | — |
| [15 — Gate 5, the prototypes](story/15-gate-5-prototypes.md) | Three complete home pages with three different bets, side by side; a second frontier model debates them; the AI Builder picks A and merges what B and C did better | `gate-5-prototypes` |
| [14 — References, shown](story/14-references.md) | Ten reference pages fetched and rendered; the AI Builder picks four; the bar torn down again into ten mechanisms | — |
| [13 — Loop 2, the grilling](story/13-loop-2-the-grilling.md) | Rounds 3 (taste) and 4 (verifiers), performed: Priya's sentence, the bar as named pages, the close-the-tab list, phone first, the verifiers | — |
| [12 — The verdict](story/12-the-verdict.md) | "You totally spoiled the experience": the grilling and the choosing were missing; what happened, in two voices; the flow corrected in every artifact; a salvage menu for the AI Builder | — |
...
The overnight profile was run five times on the laptop and stopped at step 109 of 450 for a power cut — without a checkpoint, which the AI Builder rightly called ML 101 (D-039). Training was closed (D-037), reopened with periodic checkpoints (D-040), and attempt six stopped at step 9 without a checkpoint; the AI Builder moved fine-tuning to the night and the day to the app (D-043). Measured before relaunch (2026-09-09): not a crash — no error event, a clean stderr — the overnight profile checkpoints every 25 steps at about 43 s a step, so the pause lost six minutes by design. The night's first launch (02:35 UTC, chapter 18) died 78 s in with an access violation in torch's CPU library during the model load, at 9.8 GB of commit headroom against the ~12 GB the load peaks at — the pre-flight now refuses below that floor instead of warning (D-054), and the relaunch waited for memory only the human could free. The third launch, at a floor the AI Builder lowered to 9 GB, ran to the end: 450 steps in 4 h 38 min, loss 0.0034, model `dc95336b` — the first overnight run to finish on this machine. Its evaluation then read field F1 0.53 against the delivered model's 0.95, and that number was measured before it was believed (D-055): the "100 test documents" were the head of the split in path order, all CORD receipts, and the evaluator had scored 534 predictions against fields CORD never annotates as false positives — rule 7's "unknown, not null" held in the trainer and not on the scoreboard. Both defects were fixed test-first; the two models are being re-measured on the same sampled hundred, and the pin is the AI Builder's call from those numbers. The run trains on the 5,000-item `overnight-auto` dataset with the unknown-field mask (D-030) and the leaner trainer (D-036). Until its numbers land in chapter 11, the demo adapter above is the delivered model. Next loop's first items: `lab/intent/eval-vendor-name-unseen-vendor.md` and `lab/intent/eval-errors-grounded-on-the-page.md`.
...
## Loop 2: the grilling, the references, the picks (2026-09-08/09)

Then the verdict (chapter 12): the AI Builder had expected to be grilled and to choose from rendered prototypes, and neither had happened. The flow was corrected in every skill and in `CLAUDE.md` (D-048), the design video re-read from its transcript and applied in full (D-049), a Series-C bar written for the home page (D-050), and the AI Builder chose to rewind at the plan gate, live — with Fable playing the elite AI Builder's answers in the story and the human directing from outside (D-051). Chapters 13–16 are that loop: four rounds of grilling in the character's voice (Priya, the bar, the close-the-tab list, phone first, the verifiers); references fetched from galleries and shown; three complete home pages rendered side by side, debated by a second frontier model with fresh context, **A** picked with merges from B and C and rebuilt as the live `/` (`gate-5-prototypes`); three review-screen directions debated the same way, **B, the calm document,** picked with the debate's one change — what needs a person, under the verdict, above the page, with its add and edit right there (`gate-5-review`); a taste review on the running phone; and two fresh finance-lead readers on the rebuilt home page, one of whom failed it on a cut caption — fixed as round 15 with the AI Builder's word list. Then a second fresh customer walked the rebuilt site on desktop and phone (chapter 17) and found six broken things the critics and both test suites had passed — the worst was silence: an upload of a file already in the queue was accepted, idempotently and correctly, and the page said nothing. One boolean and one sentence later it says "already in your queue as …, open it"; the phone menu opens, every footer link lands, the pricing page fits a phone, and the AI Builder ruled on eighteen confusing lines one by one. The one measure the agent cannot take alone is now the last rule: did the AI Builder choose this, from options they saw?
